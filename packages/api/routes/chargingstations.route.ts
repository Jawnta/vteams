...
router.get('/zone/:chargingZoneId', async (req, res) => {
    ...
    try {
        const result = await chargingStations.getZoneChargingZoneId(chargingZoneId);
        res.status(result.status || 200).send(result.data);
    }
    catch (err) {
        return res.status(500).send({
...
router.get('/:chargingStationId', async (req, res) => {
    ...
    try {
        const result = await chargingStations.getChargingStationId(chargingStationId);
        res.status(result.status || 200).send(result.data);
    }
    catch (err) {
        return res.status(500).send({
...
router.delete('/:chargingStationId', async (req, res) => {
    ...
    try {
        const result = await chargingStations.deleteChargingStationId(chargingStationId);
        res.status(result.status || 200).send(result.data);
    }
    catch (err) {
        return res.status(500).send({
